...
func logQuotaDataCache(userId int, username string, tokenName string, modelName string, quota int, createdAt int64, tokenUsed int) {
...
func LogQuotaData(userId int, username string, tokenName string, modelName string, quota int, createdAt int64, tokenUsed int) {
	// 只精确到小时
	createdAt = createdAt - (createdAt % 3600)

	CacheQuotaDataLock.Lock()
	defer CacheQuotaDataLock.Unlock()
	logQuotaDataCache(userId, username, tokenName, modelName, quota, createdAt, tokenUsed)
}
...
func SaveQuotaDataCache() {
	...
	for _, quotaData := range CacheQuotaData {
		...
		DB.Table("quota_data").Where("user_id = ? and username = ? and token_name = ? and model_name = ? and created_at = ?",
			quotaData.UserID, quotaData.Username, quotaData.TokenName, quotaData.ModelName, quotaData.CreatedAt).First(quotaDataDB)
		if quotaDataDB.Id > 0 {
			//quotaDataDB.Count += quotaData.Count
			//quotaDataDB.Quota += quotaData.Quota
			//DB.Table("quota_data").Save(quotaDataDB)
			increaseQuotaData(quotaData.UserID, quotaData.Username, quotaData.TokenName, quotaData.ModelName, quotaData.Count, quotaData.Quota, quotaData.CreatedAt, quotaData.TokenUsed)
		} else {
...
func increaseQuotaData(userId int, username string, tokenName string, modelName string, count int, quota int, createdAt int64, tokenUsed int) {
	err := DB.Table("quota_data").Where("user_id = ? and username = ? and token_name = ? and model_name = ? and created_at = ?",
		userId, username, tokenName, modelName, createdAt).Updates(map[string]interface{}{
