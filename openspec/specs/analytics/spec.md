...
## Purpose
TBD - created by archiving change add-token-analytics. Update Purpose after archive.
## Requirements
...
### Requirement: Token Details Drill-down
The system SHALL allow users to view detailed analytics (including Model Distribution) for a specific token.

#### Scenario: Navigate to Token Detail
- **WHEN** the user clicks on a Token (in the chart or list)
- **THEN** the system navigates to the Dashboard page filtered by that Token.
- **AND** the Dashboard displays the Model Distribution, Trend, and other stats for that specific Token.
