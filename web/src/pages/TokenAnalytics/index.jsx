...
import ApiInfoPanel from '../../components/dashboard/ApiInfoPanel';
...
const TokenAnalytics = () => {
  ...
  const handleChartClick = useCallback((params) => {
    // Check if we clicked on a model (which is token in this context)
    // The params structure depends on VChart event params.
    // Usually params.datum contains the data item.
    // Our data item has 'Model' field populated with token name.
    const datum = params?.datum;
    let tokenName = '';

    if (datum) {
      if (datum.Model) {
        tokenName = datum.Model;
      } else if (datum.type) {
        // Pie chart uses 'type'
        tokenName = datum.type;
      }
    }

    if (tokenName) {
      // Navigate to Dashboard with token_name filter
      // dashboardData.navigate is from useNavigate
      dashboardData.navigate(`/console?token_name=${encodeURIComponent(tokenName)}`);
    }
  }, [dashboardData.navigate]);
  ...
  return (
    <div className='mt-[60px] px-2 h-full'>
      <DashboardHeader
